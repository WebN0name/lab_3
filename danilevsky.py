...
def get_l_l(P):
    ...
    print(f'np root : {tmp}')
    roots = np.array([r.real for r in tmp if abs(r.imag) < 0.5])
    roots.sort()
    print(roots)
    ...
    while i < L:
        ...
        i += 1
        while i < L and abs(ret[-1]['r']-roots[i]) < 0.5:
            i += 1
            if abs(ret[-1]['r']-roots[i]) < 0.5:
                ret[-1]['r'] = (ret[-1]['r'] + roots[i])/2
            ret[-1]['k'] += 1
    return ret
